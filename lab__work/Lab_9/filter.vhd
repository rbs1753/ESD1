...
Entity filter is
  port (
        ...
        write      : in std_logic;                       --This is enables the internal registers and coincides with a new audio sample
        data_out   : out std_logic_vector(15 downto 0)    --This is the filtered audio signal out, in 16 bit fixed point format
        );
end filter;

architecture model of filter is
...
constant Lvalues: sample := (x"0051", x"00BA", x"01E1", x"0408", x"071A", x"0AAC", x"0E11", x"107F", x"1161", x"107F", x"0E11", x"0AAC", x"071A", x"0408", x"01E1", x"00BA", x"0051"); --Low pass constants
constant Hvalues: sample := (x"003E", x"FF9B", x"FE9F", x"0000", x"0535", x"05B2", x"F5AC", x"DAB7", x"4C91", x"DAB7", x"F5AC", x"05B2", x"0535", x"0000", x"FE9F", x"FF9B", x"003E"); --High pass constants

signal data_shift: sample:= (others => (others => '0')); --Array to hold the data coming in
...
begin
	...
	filter_select: process(clk, reset_n, write, address)
	begin
	  if(reset_n = '0') then
	    switch <= (others => '0');
